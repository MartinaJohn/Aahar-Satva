  ...
  const approveProduct = async (qrCodeResult) => {
    try {
      ...
      if (typeof qrCodeResult === 'string' && ethers.utils.isHexString(qrCodeResult)) {
        ...
        const result = await contract.approveProductByFoodSafetyOfficer(txHashBytes32);
        console.log(result);
      } else {
  ...
  return (
    <div>
     ...
     <label>Latitude</label>
      <input value={position.latitude} readOnly />
      <label>Longitude</label>
      <input  value={position.longitude} readOnly />

      <a href={`https://www.google.com/maps/search/?api=1&query=${position.latitude},${position.longitude}`} target="_blank" rel="noreferrer">
        GPS Link
      </a>
    </div>
  );
